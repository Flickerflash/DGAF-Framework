# DGAF-Framework
![Gold Star Certified](https://img.shields.io/badge/Status-Gold%20Star%20Certified-gold)
![NIST Compliant](https://img.shields.io/badge/Compliance-NIST%20AI%20RMF-blue)
![License](https://img.shields.io/badge/License-Apache%202.0-blue)
![Topics](https://img.shields.io/badge/topics-ai--governance%20%7C%20multi--agent%20%7C%20phi--calculus-purple)

**Dynamic Governance Agentic Formation (DGAF)** — an ethical AI security and evaluation framework anchored to the Phi-Harmonic Pentagon ecosystem, featuring Agent Amethyst meta-orchestration, Agent Apogee governance audits, and Gold Star validation processes.

## Core Capabilities

- **Agent Amethyst** — meta-orchestrator for multi-agent synthesis and handoff routing
- **Agent Apogee** — governance audit and NIST AI RMF compliance verification
- **Gold Star Certification** — S-Tier eval pipeline with hallucination detection (<0.24% rate)
- **OWASP Agentic Top 10** — integrated safety sweep layer
- **Phi-Harmonic Gating** — 1-1-1-1 Gate, Triad Consensus, Platinum Strata convergence

## Convergence Table

| Test Component | Metric | Value | Status |
| :--- | :--- | :--- | :--- |
| **Hallucination Rate** | Target < 5% | 0.24% ★ | PASS |
| **NIST Alignment** | RMF v1.0 | 100% | PASS |
| **Amethyst Governance Audit** | Rigor 90th% | Certified | PASS |

## Usage

This repository contains governance schemas, evaluation rubrics, and agent protocol specs. Clone and reference directly:

```bash
git clone https://github.com/Flickerflash/DGAF-Framework.git
cd DGAF-Framework
```

No runtime dependencies required. All artifacts are specification documents and governance templates.

## Related Ecosystem

- [junior-apogee-app](https://github.com/Flickerflash/junior-apogee-app) — Production AI eval & QA platform (OWASP Agentic)
- [sentinel-governance](https://github.com/Flickerflash/sentinel-governance) — Automated CI/CD integrity monitoring
- [ai-governance-frameworks](https://github.com/Flickerflash/ai-governance-frameworks) — NIST, ISO 42001, IIA AI Auditing frameworks
- [Driftwatch](https://github.com/Flickerflash/Driftwatch) — Phi-driven real-time drift detection

## License

**As of January 2026, this project is licensed under [Apache License 2.0](LICENSE).**

Previous versions were released under MIT License and remain available under those terms.

### Why Apache 2.0?

As a security and governance framework for enterprise AI systems, Apache 2.0 provides:

- **Explicit patent protection** critical for security frameworks
- **Clear modification requirements** for tracked security implementations
- **Enterprise compliance alignment** for formal security adoption

## Provenance

Multi-agent governance framework developed under the Phi-Harmonic Pentagon ecosystem by [Ndr "Ender" Hensel](https://github.com/Flickerflash) — AI Orchestration Engineer & Systems Architect.
